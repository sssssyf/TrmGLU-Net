...
class FeedForward(nn.Module):
    def __init__(self, dim, pca_dim,mult = 4):
        super().__init__()
        hidden_dim = dim * mult
        ...
        self.project_in_skip = nn.Conv2d(2*dim, hidden_dim, 1)
        self.project_in_skip2 = nn.Conv2d(dim+pca_dim, hidden_dim, 1)

    def forward(self, x, skip = None,skip2 = None,time_emb = None):
        if exists(skip):

            x=torch.cat((x,skip),dim=1)
            x=self.project_in_skip(x)
        elif exists(skip2):
            x = torch.cat((x, skip2), dim=1)
            x=self.project_in_skip2(x)
        else :
            x = self.project_in(x)
        if exists(time_emb):
            time_emb = rearrange(time_emb, 'b c -> b c () ()')
    ...
    def __init__(
        ...
        for _ in range(depth):
            self.layers.append(List([
                ...
                PreNorm(dim, FeedForward(dim, pca_dim,mult = ff_mult))
            ]))

    def forward(self, x, skip = None, skip2= None, time = None):
        attn_time_emb = None
        ff_time_emb = None
        if exists(time):
        ...
        for attn, ff in self.layers:
            ...
            x = ff(x,skip = skip, skip2=skip2,  time_emb = ff_time_emb) + x
        return x
...
class Uformer(nn.Module):
    def __init__(
        ...
        self.project_out = nn.Sequential(
            nn.Conv2d(2*dim+3, output_channels, 3, padding = 1),
        )
        ...
        for ind, heads, window_size, dim_head, num_blocks in zip(range(stages), heads, window_size, dim_head, num_blocks):
            ...
            self.downs.append(List([
                Block(dim, depth = num_blocks, dim_head = dim_head, heads = heads, ff_mult = ff_mult, window_size = window_size, time_emb_dim = time_emb_dim,pca_dim=pca_dim),
                nn.Conv2d(dim, dim * 2, 4, stride = 2, padding = 1),
                nn.Conv2d(pca_dim, pca_dim * 2, 4, stride=2, padding=1)
            ]))
    ...
    def forward(self,x,time = None):
        ...
        skip2=original
        trm_skips = []
        for block, downsample,pca_downsample in self.downs:
            x = block(x,skip2=skip2, time = time)
            trm_skips.append(x)
            x = downsample(x)
            skip2=pca_downsample(skip2)

        x = self.mid(x, time = time)

        for (upsample, block), trm_skip in zip(reversed(self.ups), reversed(trm_skips)):
            x = upsample(x)
            x = block(x, skip = trm_skip, time = time)

        x = self.project_out(torch.cat((x,residual,original),dim=1))
        return x
